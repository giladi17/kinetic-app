...
export default function Dashboard() {
  ...
  const proteinTarget = macros?.protein?.target  ?? 130
  ...
  const calories      = macros?.calories?.consumed ?? 0
  const calTarget     = macros?.calories?.target   ?? 2800
  ...
  return (
    <main className="mt-20 md:mt-24 px-4 md:px-8 max-w-7xl mx-auto pb-32 bg-[#F8F9FF] dark:bg-[#0e0e0e] text-[#151C25] dark:text-white dark:text-white min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8">
        ...
        {/* ⑥ Weekly Nutrition Chart — full width */}
